...
class ConduitMarkdownPreprocessor {
  ...
  /// Repairs collapsible `<details>` blocks whose tags were HTML-escaped
  /// (`&lt;details&gt;` … `&lt;summary&gt;…&lt;/summary&gt;` … `&lt;/details&gt;`).
  ///
  /// The reasoning/tool-call/code-interpreter collapsible renderer only
  /// recognizes PLAIN `<details>` tags. A `<details>` block that was HTML-escaped
  /// before display would otherwise show the raw `&lt;details&gt;` markup instead
  /// of a collapsed section. Two real cases produce this:
  /// 1. A reloaded assistant reasoning turn re-wrapped as escaped text
  ///    (`&lt;details type="reasoning"...&gt;`).
  /// 2. An OpenWebUI image-generation pipe emitting a BARE, escaped
  ///    `&lt;details&gt;&lt;summary&gt;思考用时 (Ns)&lt;/summary&gt;…` block
  ///    (no `type` attribute).
  ///
  /// Detection keys on an escaped `<details>` that CONTAINS an escaped
  /// `<summary>` — the shared trait of every collapsible block — so ordinary
  /// escaped `&lt;details&gt;` text a user pasted (no `<summary>`) is never
  /// touched. Only the tag scaffolding is unescaped; body text between the tags
  /// is left as-is (it may carry legitimately-escaped entities, including
  /// double-escaped `&amp;gt;` blockquote markers, that the details renderer
  /// unescapes itself at parse time).
  static String repairEscapedSemanticDetails(String input) {
    // Fast path: need an escaped opener AND an escaped summary somewhere.
    if (!input.contains('&lt;details') || !input.contains('&lt;summary&gt;')) {
      return input;
    }

    // Match a full escaped block that carries an escaped <summary> (collapsible
    // trait). Non-greedy body; require the escaped </details> closer.
    final blockPattern = RegExp(
      r'&lt;details\b[\s\S]*?&lt;summary&gt;[\s\S]*?&lt;/summary&gt;[\s\S]*?&lt;/details&gt;',
      caseSensitive: false,
    );

    return input.replaceAllMapped(blockPattern, (match) {
      final block = match.group(0)!;
      // Unescape ONLY the scaffolding tags inside this block, not the body.
      return block
          // Opener `&lt;details ...&gt;` -> `<details ...>` (attrs may use
          // `&quot;`; convert those quotes so type/duration parse works).
          .replaceAllMapped(
            RegExp(r'&lt;details\b([\s\S]*?)&gt;', caseSensitive: false),
