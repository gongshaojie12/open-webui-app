...
class Pipe:
    ...
    class UserValves(BaseModel):
        ENABLE_GOOGLE_SEARCH: bool = Field(
            default=True,
            description="是否启用 Google 搜索（返回 groundingMetadata 与引用）",
        )
        IMAGE_GENERATION_ASPECT_RATIO: str = Field(
            default="16:9",
            description="用于定义生成图片的宽高比例,(注: 1:4,4:1,1:8,8:1仅适用于gemini-3.1-flash-image-preview).",
            json_schema_extra={"enum": ASPECT_RATIO_OPTIONS},
        )
        IMAGE_GENERATION_RESOLUTION: str = Field(
            default="1K",
            description="用于定义生成图片的分辨率,(注: 1:4,4:1,1:8,8:1仅适用于gemini-3.1-flash-image-preview).",
    ...
    class Valves(BaseModel):
        ...
        BUILTIN_MODELS: str = Field(
            default=os.getenv("GOOGLE_BUILTIN_MODELS", ""),
            description="Defines the built-in Google models available for use.",
        )
    ...
    def get_google_models(self, force_refresh: bool = False) -> List[Dict[str, str]]:
        ...
        try:
            ...
            # print(f"1.15.2 image models====================={models}")
            ...
            # print(f"1.15.2 image model map firse================{model_map}")
                    ...
                    for loc in fallback_locations:
                        try:
                            ...
                            for m in alt_models:
                                ...
                                vid = self._check_video_generation_support(mid)
                                name = disp or mid
                                if img:
                                    name += " 🎨"
                                if vid:
                            ...
                            pass
            # print(f"1.15.2 image model map second================{model_map}")
            builtin = [
                m.strip()
            ...
            for mid in builtin:
                ...
                if not mid.startswith("gemini-") and not mid.startswith("veo-"):
                    continue
                if mid not in model_map:
                    img = self._check_image_generation_support(mid)
                    vid = self._check_video_generation_support(mid)
                    if mid.startswith("veo-"):
                        name = mid.replace("veo-", "Veo ").replace("-", " ").title()
                    else:
                        name = (
                            mid.replace("gemini-", "Gemini ").replace("-", " ").title()
                        )
                    if img:
                        name += " 🎨"
                    if vid:
            ...
            # print(f"1.15.2 image filtered_models==================={filtered_models}")
            self._model_cache = [
                m
                for m in filtered_models.values()
                if m.get("image_generation") is True
                or m.get("video_generation") is True
            ]
    ...
    def _check_thinking_support(self, model_id: str) -> bool:
        ...
        # Image generation models do NOT accept thinking_level via the Google API,
        # even Gemini 3 image variants like gemini-3-pro-image-preview.
        # Sending thinking_level to these models triggers a 400 INVALID_ARGUMENT.
        # (1.15.2 upstream wrongly assumed Gemini 3 image models support thinking_level;
        # we keep 1.11.1's conservative behavior: any image-generation/preview model
        # skips the thinking config entirely.)
        if "image" in model_id.lower() and (
            "generation" in model_id.lower() or "preview" in model_id.lower()
        ):
            return False
    ...
    def pipes(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            print(f"pipes============models======>{self.get_google_models()}")
    ...
    def _configure_generation(
        ...
        if enable_image_generation:
            ...
            print(f"1.15.2 image ==================_check_image_config_support before")
            if self._check_image_config_support(model_id):
                print(
                    f"1.15.2 image ==================_check_image_config_support true"
                )
                ...
                print(
                    f"1.15.2 image===========validated_aspect_ratio：{validated_aspect_ratio}  validated_resolution：{validated_resolution}"
                )
        ...
        if __tools__ is not None and params.get("function_calling") == "native":
            for name, tool_def in __tools__.items():
                # Gemini 图像生成响应只允许 google_search / maps grounding 工具，
                # 任何普通 function-calling 工具（含 Open WebUI 内置工具）都会导致
                # 400 "Only google search tool and maps imagery grounding tool is
                # supported for image response."。因此图像生成时一律跳过这些工具。
                # grounding 工具在前面已单独 append，不受此影响，故图像生成 + 联网搜索仍可共存。
                if enable_image_generation:
                    self.log.debug(
                        f"Skipping function-calling tool '{name}' for native Gemini image generation"
                    )
                    continue
...
{quoted_content}
    ...
    async def _generate_video(
        ...
        await emit_status(f"开始使用 {model_id} 生成视频...", False)
        ...
        try:
            ...
            await emit_status(f"视频生成失败: {e}", True)
            return f"启动视频生成时出错: {e}"
        ...
        while not operation.done:
            ...
            if poll_timeout > 0 and elapsed >= poll_timeout:
                error_msg = (
                    f"视频生成超时，已用时 {elapsed} 秒 " f"(限制: {poll_timeout} 秒)"
                )
            ...
            await emit_status(f"正在生成视频... (已用时 {elapsed} 秒)", False)

        if operation.error:
            error_msg = str(operation.error)
            self.log.error(f"Video generation failed: {error_msg}")
            await emit_status(f"视频生成失败: {error_msg}", True)
        ...
        await emit_status(f"视频生成完成 (用时 {elapsed} 秒)", True)
    ...
    async def pipe(
        ...
        print(f"1.15.2 image user_valves============================{user_valves}")
        try:
            # Parse and validate model ID
            model_id = body.get("model", "")
            if "gemini-3-pro-image-preview" in model_id and user_valves is not None:
                if getattr(user_valves, "IMAGE_GENERATION_ASPECT_RATIO", None) in [
                    "1:4",
                    "4:1",
                    "1:8",
                    "8:1",
                    ...
                    await __event_emitter__(
                        {
                            "type": "message",
                            "data": {
                                "content": "\n\n⚠️ 提示：当前模型只支持['1:1','2:3','3:2','3:4','4:3','4:5','5:4','9:16','16:9','21:9']，不支持 '{}'。请重新配置Image Generation Aspect Ratio。".format(
                                    user_valves.IMAGE_GENERATION_ASPECT_RATIO
                                )
                            },
                        }
                    )
                ...
                if getattr(user_valves, "IMAGE_GENERATION_RESOLUTION", None) == "512":
                    await __event_emitter__(
                        {
                            "type": "message",
                            "data": {
                                "content": "\n\n⚠️ 提示：当前模型只支持['1K','2K','4K']，不支持 '{}'。请重新配置Image Generation Resolution。".format(
                                    user_valves.IMAGE_GENERATION_RESOLUTION
                                )
                            },
                        }
                    )
            ...
            print(
                f"1.15.2 image generation_config================================>{generation_config}"
            )
            ...
            if not stream or supports_image_generation:
                try:
                    ...
                    if supports_image_generation and __event_emitter__ is not None:
                        await __event_emitter__(
                            {
                                "type": "status",
                                "data": {
                                    "action": "image_processing",
                                    "description": "正在处理图像请求...",
                                    "done": False,
                                },
                            }
                        )
                    ...
                    for part in parts:
                        if getattr(part, "thought", False) and getattr(
                            ...
                            # gemini-3-image 模型一次生成一张，但是 3.1 会生成两张，只取一张即可
                    ...
                    if thought_segments:
                        ...
                        details_block = f"""<details>
<summary>思考用时 ({duration_s}s)</summary>
...
{quoted_content}
                    ...
                    # Return content as plain string so Open WebUI persists the message
                    # content (markdown image) to DB. Returning a dict here caused only
                    # status indicators to be saved, so the image disappeared after the
                    # user switched chats and came back. Matches 1.11.1 working behavior.
